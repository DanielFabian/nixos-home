...
And most importantly; we argee on semantics and design first carefully first, understanding what invariants are latent in the code and exploited. Let's name them explicitly. So a change consists of _first_ full reverse engineering of the current semantics, then a proposal of the new semantics, and only then we start coding.
